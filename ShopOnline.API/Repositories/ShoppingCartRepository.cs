...
namespace ShopOnline.API.Repositories
{
    public class ShoppingCartRepository : IShoppingCartRepository
    {
        ...
        {
            // check that item isn't already in the cart
            if (await CartItemExists(cartItemToAddDto.CartId, cartItemToAddDto.ProductId) == false){

                var itme = await (from product in _context.Products where product.Id == cartItemToAddDto.ProductId
                select new CartItem
                {
                ...
                if (itme != null)
                {
                    var result = await _context.CartItems.AddAsync(itme);
